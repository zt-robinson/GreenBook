#!/usr/bin/env python3
"""
Create and store a single standard event
"""
...
from core.tournament_naming import generate_standard_event_name
...
def get_next_week_number(season_number):
    tournaments_db_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'golf_tournaments.db')
    conn = sqlite3.connect(tournaments_db_path)
    cur = conn.cursor()
    cur.execute('SELECT COUNT(*) FROM tournaments WHERE season_number = ?', (season_number,))
    count = cur.fetchone()[0]
    conn.close()
    return count + 1

def create_standard_event(tournament_name: str, course_id: int, start_date: str, 
                         season_number: int, week_number: int):
    """Create a single standard event"""
    
    print(f"🎯 Creating Standard Event: {tournament_name}")
    print("=" * 50)
    ...
    try:
        ...
        print(f"\n✅ Standard event '{tournament_name}' created successfully!")
        print(f"   Tournament ID: {tournament_id}")
        print(f"   Event Type: standard")
        ...
        print(f"❌ Error creating standard event: {e}")
        return None
...
def main():
    """Main function to create a standard event"""
    
    print("🎯 Standard Event Creator")
    print("=" * 30)
    ...
    week_number = get_next_week_number(season_number)
    
    print(f"\n📝 Creating standard event with parameters:")
    print(f"   Name: {tournament_name}")
    ...
    tournament_id = create_standard_event(
        tournament_name=tournament_name,
        course_id=course_id,
        start_date=start_date,
        season_number=season_number,
    ...
    if tournament_id:
        print(f"\n🎉 Standard event created successfully!")
        print(f"   Ready to be used in tournament simulation")
    else:
        print(f"\n❌ Failed to create standard event")
