#!/usr/bin/env python3
"""
Create and store The American Open major
"""
...
def get_available_courses_for_season(season_number):
    ...
    cur = conn.cursor()
    # Only US courses for The American Open
...
def create_american_open(course_id: int, start_date: str, season_number: int, week_number: int):
    """Create The American Open major"""
    
    print(f"🎯 Creating The American Open")
    print("=" * 50)
    ...
    try:
        ...
        overrides = {
            ...
            'cut_line': {
                'type': 'position',
                'value': 67,
                'description': 'Top 67 and ties advance to weekend'
            },
        ...
        tournament_id = tournament_logic.create_tournament(
            tournament_name="The American Open",
            course_id=course_id,
        ...
        print(f"\n✅ The American Open created successfully!")
        ...
        print(f"❌ Error creating The American Open: {e}")
        return None

def main():
    """Main function to create The American Open"""
    
    print("🏆 The American Open Creator")
    print("=" * 35)
    ...
    # Get the next event date and week number
    start_date, week_number = get_next_event_date_and_week(season_number)
    if start_date is None:
        return
    
    print(f"\n📝 Creating The American Open with parameters:")
    print(f"   Name: The American Open")
    print(f"   Course: {course_name} ({state_country})")
    print(f"   Date: {start_date}")
    print(f"   Season: {season_number}, Week: {week_number}")
    
    # Create the major
    tournament_id = create_american_open(
        course_id=course_id,
        start_date=start_date,
        season_number=season_number,
        week_number=week_number
    ...
    if tournament_id:
        ...
        print(f"\n❌ Failed to create The American Open")
