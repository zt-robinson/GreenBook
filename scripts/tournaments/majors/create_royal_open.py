#!/usr/bin/env python3
"""
Create and store The Royal Open major
"""
...
def get_available_courses_for_season(season_number):
    """Get list of available UK courses for the season (exclude already used)"""
    ...
    cur = conn.cursor()
    # Only UK courses for The Royal Open
    cur.execute("SELECT id, name, state_country, location FROM courses WHERE state_country LIKE '%UK%' OR location LIKE '%UK%' OR location LIKE '%England%' OR location LIKE '%Scotland%' OR location LIKE '%Wales%' ORDER BY name")
...
def select_course(courses):
    ...
    print(f"\n📋 Available UK Courses ({len(courses)} total):")
    print("-" * 60)
    
    for idx, (course_id, course_name, state_country, location) in enumerate(courses, 1):
        # Extract country from location for UK courses
...
def create_royal_open(course_id: int, start_date: str, season_number: int, week_number: int):
    """Create The Royal Open major"""
    
    print(f"🎯 Creating The Royal Open")
    print("=" * 50)
    ...
    try:
        ...
        tournament_id = tournament_logic.create_tournament(
            tournament_name="The Royal Open",
            course_id=course_id,
        ...
        print(f"\n✅ The Royal Open created successfully!")
        ...
        print(f"❌ Error creating The Royal Open: {e}")
        return None

def main():
    """Main function to create The Royal Open"""
    
    print("🏆 The Royal Open Creator")
    print("=" * 35)
    ...
    if not courses:
        print(f"❌ No available UK courses left for season {season_number}")
        return
    
    print(f"📋 Found {len(courses)} available UK courses for season {season_number}")
    ...
    print(f"\n📝 Creating The Royal Open with parameters:")
    print(f"   Name: The Royal Open")
    # Extract country from location for UK courses
    if location and ',' in location:
    ...
    # Create the major
    tournament_id = create_royal_open(
        course_id=course_id,
        start_date=start_date,
        season_number=season_number,
        week_number=week_number
    ...
    if tournament_id:
        ...
        print(f"\n❌ Failed to create The Royal Open")
